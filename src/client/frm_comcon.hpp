...
class frm_comcon:public frm_base {
  Q_OBJECT;

public:
  frm_comcon( QWidget*parent=0, const char*name=0, bool modal=true, WFlags fl=MYFLAGS );
  ~frm_comcon();
  ...
  bool scambia;         // indica di scambiarsi conto<->comanda
  void contanti();
  /* cerca e seleziona la forma di pagamento in contanti oppure l'ultima in elenco */
  ...
  void stato_pulsanti(); // abilita disabilita pulsanti di competenza

  // stampa
  enum tipo_doc { td_ricevuta, td_scontrino, td_fattura, td_proforma, td_concordato, td_devnul };
...
private:
  ...
  enum t_comanda { c_comanda, c_separata, c_pagamento };

  void aggiorna( t_comanda controllo, bool solo_stato=false ); // solo_stato vuol dire solo lo stato della form: pulsanti validi etc.
  void aggiorna( bool solo_stato=false );
  bool _divisione_fittizia;

  /* combinato con comanda->modalita()==m_conti indica se la form
  �in stato (A) dividi_conto solo a video, (B) dividi conto registrato */
  bool multi_conto() { // di utilita' x identificare il conto diviso
    return comanda->modalita()==tr_comanda::m_conti?true:false;
  }

  bool separato() { return separata->righe(); } // indica se c'�un conto separato in corso
